...
//dd($request);
       ...
       try{
            ...
            $UserRequest->braintree_nonce=$request->clientToken ? : "";
    ...
    public function modifiy_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'payment_mode' => 'sometimes|nullable|in:CASH,CARD,PAYPAL,SQUARE',
                'card_id' => ['required_if:payment_mode,CARD','exists:cards,card_id,user_id,'.Auth::user()->id],
            ]);
    ...
    public function wallet_transfer(Request $request)
    {
        ...
        {
            if($request->ajax()){    
            return response()->json(['message' => 'Your wallet balance to low, so you cant transfer'],422);    
            }
            else
            {
                return back()->with('flash_success','Your wallet balance to low, so you cant transfer');
            } 
        }
      ...
      try{
            ...
            $UserWallet->transaction_id=rand(100000,9999999);
            $UserWallet->transaction_desc=Auth::user()->first_name.' is transfer wallet amount';
            $UserWallet->type='C';
            ...
            $User->transaction_id=rand(100000,9999999);
            $User->transaction_desc='Wallet amount transfered to '.$user->first_name;
            $User->type='D';
            $User->amount=$request->amount;
            $User->open_balance=Auth::user()->wallet_balance;
            $User->close_balance=Auth::user()->wallet_balance-$request->amount;
            $User->save();

            $UserUpdate = Auth::user(); 
            $UserUpdate->wallet_balance = Auth::user()->wallet_balance-$request->amount;
            $UserUpdate->save();

            if($request->ajax()){    
            return response()->json(['message' => 'Wallet amount transfered is successfully']);    
            }
            else
            {
                return back()->with('flash_success','Wallet amount transfered is successfully');
            }  
        } catch(Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }  
    ...
    public function braintree_flow(Request $request){
        ...
        return view('user.ride.braintree',compact('clientToken','request','amount','tip_amount'));

    }

    public function square_flow(Request $request){
        ...
        return view('user.ride.square',compact('request','amount','tip_amount'));
